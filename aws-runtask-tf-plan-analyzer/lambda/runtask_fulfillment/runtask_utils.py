...
def generate_runtask_result(outcome_id, description, result, status):
    # Determine the tags and severity based on status
    if status == "passed":
        tags = {
            "status": [{"label": "Passed", "level": "info"}],
            "severity": [{"label": "Info", "level": "info"}],
        }
    elif status == "failed":
        tags = {
            "status": [{"label": "Error", "level": "error"}],
            "severity": [{"label": "Critical", "level": "error"}],
        }
    else:
        raise ValueError(f"Unknown status: {status}")

    # Create the result JSON with the appropriate tags
